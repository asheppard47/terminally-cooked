...
Known v1 simplifications, on purpose: the Grok adapter reads `events.jsonl` + `updates.jsonl` only (no `chat_history.jsonl`), emits a human turn per `turn_started`, and carries no command text (Doom Loop and test tallies fail open there); the Gemini adapter treats statuses outside its map as unknown. Full advisory transcripts were session-scratchpad artifacts; this doc is the durable record.
